...
import {
    alphabeticalGenerator, choice, fixRandom, getRandomInteger, getRealRandomInteger,
} from "../util/random";
...
export default class Transform
{
    ...
    get className()
    {
        return (ObfuscateOrder[this.priority] || (this as any).__proto__.constructor.name);
    }
    ...
    input(object: Node, parents: Node[]): ExitCallback | void
    {
        if (this.match(object, parents))
        {
            if (Array.isArray(object))
            {
                ...
                console.log(`transform type= ${JSON.stringify(object, null, " ")}`)
            }
    ...
    getPlaceholder()
    {
        const genRanHex = (size) => [...Array(size)]
            .map(() => Math.floor(fixRandom() * 10).toString(10))
            .join("");
    ...
    generateIdentifier(length: number = -1, count = -1): string
    {
        if (length == -1)
        {
            length = getRealRandomInteger(6, 8);
        }
        ...
        do
        {
            identifier = ComputeProbabilityMap(this.options.identifierGenerator, (mode = "randomized") =>
            {
                switch (mode)
                {
                    case "randomized":
                    ...
                    case "hexadecimal":
                        const genRanHex = (size) => [...Array(size)]
                            .map(() => Math.floor(fixRandom() * 16).toString(16))
                            .join("");
                    ...
                    case "mangled":
                        while (1)
                        {
                            ...
                            if (reservedKeywords.has(result) || reservedIdentifiers.has(result))
                            {
                            } else
                            {
                                return result;
